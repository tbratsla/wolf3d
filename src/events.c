...
void	event(t_wolf *wolf)
{
	if ((wolf->event.key.type == SDL_KEYDOWN &&
		...
		exit(1);
	if (wolf->event.key.type == SDL_KEYDOWN &&
		(wolf->event.key.keysym.sym == SDLK_w ||\
...
void	rotation(t_wolf *wolf, int side)
{
	...
	ft_clear_screen(wolf);
	old_dir_x = wolf->dirX;
	wolf->dirX = wolf->dirX * cos(-wolf->rot_speed * side) - wolf->dirY\
		* sin(-wolf->rot_speed * side);
	wolf->dirY = old_dir_x * sin(-wolf->rot_speed * side) + wolf->dirY\
		* cos(-wolf->rot_speed * side);
	old_plane_x = wolf->planeX;
	wolf->planeX = wolf->planeX * cos(-wolf->rot_speed * side) - wolf->planeY\
		* sin(-wolf->rot_speed * side);
	wolf->planeY = old_plane_x * sin(-wolf->rot_speed * side) + wolf->planeY\
		* cos(-wolf->rot_speed * side);
}
